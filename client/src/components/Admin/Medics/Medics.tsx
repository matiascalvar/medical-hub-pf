...
import style from "./Medics.module.css";
...
export default function Medics(medics: any): JSX.Element {
  ...
  const createSpecialties = (medics: any) => {
    ...
    medics.forEach((medic: any) =>
      specialtiesList.push(medic.Specialitie.name)
    );
  ...
  const updateFilter = (e: any) => {
    let filtered = [];
    if (e.target.value === "Speciality") return;
    if (e.target.value === "All") {
      filtered = medicList;
    } else {
      filtered = medicList.filter(
        (medic: any) => medic.speciality === e.target.value
      );
    }
  ...
  return (
    <div className={style.medicsContainer}>
      <h2>Medics</h2>
      {detailed ? (
        <MedicsDetail
          id={detailed}
        ...
        <div>
          <div className={style.filters}>
            <select onChange={updateFilter} name="sepecialities">
              ...
              <option>All</option>
              {specialties
                ? specialties.map((sp: string) => <option>{sp}</option>)
                : null}
            </select>
            <input
